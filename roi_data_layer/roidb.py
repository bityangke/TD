...
def add_twin_regression_targets(roidb):
    ...
    for im_i in xrange(num_images):
        rois = roidb[im_i]['twins']
        max_overlaps = roidb[im_i]['max_overlaps']
        max_classes = roidb[im_i]['max_classes']
        roidb[im_i]['twin_targets'] = \
...
#        for im_i in xrange(num_images):
#            targets = roidb[im_i]['twin_targets']
...
#        stds = np.sqrt(squared_sums / class_counts - means ** 2)
    ...
    if cfg.TRAIN.TWIN_NORMALIZE_TARGETS:
        ...
        for im_i in xrange(num_images):
            ...
            for cls in xrange(1, num_classes):
                cls_inds = np.where(targets[:, 0] == cls)[0]
                roidb[im_i]['twin_targets'][cls_inds, 1:] -= means[cls, :]
                roidb[im_i]['twin_targets'][cls_inds, 1:] /= stds[cls, :]
    else:
